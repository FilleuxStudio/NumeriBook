...
namespace PPE
{
    public partial class FRM_NumeriBook : Form
    {
                ...
                try
                {
                    ...
                    if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        foreach (Livres index in GetAllSelectedLivres())
                        {
                            ...
                            string selected = index.id_livre;
                            ...
                            string MainFolderName = index.titre;
        ...
        private void BT_Editer_Click(object sender, EventArgs e)
        {
            ...
            {
                string nom = Path.GetFileName(GetAllSelectedLivres()[0].full_chemin_livre); //Nom fichier epub
                ...
                if (charge != 3)
                {
                    FRM_Editer _Editer = new FRM_Editer(biblio, GetAllSelectedLivres()[0], charge);
                    _Editer.ShowDialog();
                }
                active.visibleForm(true);
            }
        ...
        public void SupprChoisi()
        {
...

            {
...

                {
                    ...
                    string ID = GetAllSelectedID()[i];

                    string chemin = GetAllSelectedPath()[i]; i++;

                    sourceDir = biblio.CurrentPath + chemin;

                    File.Delete(sourceDir);

                    biblio.Database.Requete("DELETE FROM A_ECRIT WHERE id_livre='" + ID + "';");

                    biblio.Database.Requete("DELETE FROM ASSOCIER_MOTSCLEF WHERE id_livre='" + ID + "';");

                    biblio.Database.Requete("DELETE FROM APPARTIENT_COLLECT WHERE id_livre='" + ID + "';");

                    biblio.Database.Requete("DELETE FROM EDITER_LIVRE WHERE id_livre='" + ID + "';");

                    biblio.Database.Requete("DELETE FROM LIVRE WHERE id_livre='" + ID + "';");
        ...
        public void SupprTout()
        {
...

            {
                ...
                int nbFichier = Directory.GetFiles(biblio.CurrentPath, "*.*", SearchOption.AllDirectories).Length;
...

                {
                    ...
                    files.Add(Directory.GetFiles(biblio.CurrentPath)[i]);
                ...
                biblio.Database.Requete("DELETE FROM A_ECRIT;");

                biblio.Database.Requete("DELETE FROM ASSOCIER_MOTSCLEF;");

                biblio.Database.Requete("DELETE FROM APPARTIENT_COLLECT;");

                biblio.Database.Requete("DELETE FROM EDITER_LIVRE;");
        ...
        public void SupprChoisiBase()
        {
...

            {
...

                {
                    ...
                    string ID = GetAllSelectedID()[i]; i++;
        ...
        private void B_SupprimerLivresBase_Click(object sender, EventArgs e)
        {
...

            {
                ...
                if (new ConfirmationContinuation().ShowDialog() == DialogResult.OK)
        ...
        private void B_SupprimerTouteBibliothèqueBase_Click(object sender, EventArgs e)
        {
            ...
            if (new ConfirmationSupprBiblio().ShowDialog() == DialogResult.OK)
        ...
        private void LV_Livre_ColumnClick(object sender, ColumnClickEventArgs e)
        {
            this.LV_Livre.ListViewItemSorter = new ListViewItemComparer(e.Column);
        }
