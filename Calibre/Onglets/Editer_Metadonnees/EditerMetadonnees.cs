    ...
    public partial class FRM_EditerMetadonnees : Form
    {
        ...
        public void ReadModification(string cheminMod, string chemin)
        {
            ...
            try
            {
                ...
                //RTB_Text.Text = binaryReader.ReadString();
                ...
                while (sQ.Read())
                {
                    ...
                    else
                    {
                        if (idNewGenre == int.Parse(sQ[0].ToString()))
                        {
                            ...
                            Database.Requete("UPDATE LIVRE SET titre = '" + TXT_Titre.Text + "', nbpages = " + int.Parse(TXT_Pages.Text) + ", evaluation = " + evaluation + ", datepubli = '" + TXT_DateBook.Text + "', description = '" + RTB_Text.Text + "', avancement = " + int.Parse(TXT_Progess.Text) + ", langue = '" + TXT_LanguesBook.Text + "', id_genre = " + idNewGenre + "  WHERE id_livre = '" + livre.id_livre + "';");
                        }
                    }
                }
                opfsql = "UPDATE LIVRE SET titre = '" + TXT_Titre.Text + "', nbpages = " + int.Parse(TXT_Pages.Text) + ", evaluation = " + evaluation + ", datepubli = '" + TXT_DateBook.Text + "', description = '" + RTB_Text.Text + "', avancement = " + int.Parse(TXT_Progess.Text) + ", langue = '" + TXT_LanguesBook.Text + "', id_genre = " + idGenre + "  WHERE id_livre = '" + livre.id_livre + "';";
                Database.Requete(opfsql);
                BookModification(); // Fichier Binaire Sauvegarde
                MessageBox.Show("Modifications apportées avec succès.");
            }
            catch (Exception ex)
            {
                ExceptionHandler.Message("Erreur de modification des métadonnées" + ex);
            }
        ...
        private void BT_PictureCoverOrg_Click(object sender, EventArgs e)
        {
            ClassEpubRead classEpubRead = new ClassEpubRead(livre.full_chemin_livre);
            PICB_Couverture.BackgroundImage = classEpubRead.picture;
            PICB_Couverture.Image = null;
        }

        private void CB_NoteBook_TextChanged(object sender, EventArgs e)
        {
            CB_NoteBook.Text = "";
        }
        ...
        private void RichtextboxBaliseHtml(string baliseStart, string baliseEnd)
        {
            ...
            RTB_Text.Text = RTB_Text.Text.Insert(SelectEnd, baliseEnd);
        }
    }
}