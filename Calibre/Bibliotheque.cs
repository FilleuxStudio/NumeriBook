    ...
    public class Bibliotheque
    {
        ...
        private void LoadBiblio()
        {
            if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + "Config.xml"))
            {
                ...
                XmlNode _CurrentPath = doc.SelectSingleNode("Bibliotheque/CurrentBib").NextSibling;
                XmlAttribute _nameAttr = _CurrentPath.FirstChild.Attributes["name"];
                SetCurrent(_CurrentPath.InnerText, _nameAttr.Value);
            }
            else
            {
        ...
        public void OpenMenu()
        {
            ...
            Dialog.LBL_CurrentPath.Text = "Emplacement Actuel : " + this.CurrentPath;
        ...
        private void Option_ChangeActual(string _selectedPath, string _name)
        {
            //Directory exists
            if (File.Exists(_selectedPath + "\\" + _name + ".sqlite"))
            {
                MessageBox.Show("Bibliothèque utilisée : " + _selectedPath, "Bibliothèque", MessageBoxButtons.OK, MessageBoxIcon.Information);
                SetCurrent(_selectedPath, _name);
            }
            else
                MessageBox.Show("Le fichier " + _selectedPath + "\\" + _name + ".sqlite n'existe pas.", "Bibliothèque", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        private void Option_MoveActual(string _selectedPath, string _name)
        {
        ...
        /// </summary>
        /// <param name="selectedPath">Chemin de destination</param>
        private void Option_CreateNew(string _selectedPath, string _name, bool _display)
        {
            Manuel manuel = new Manuel();
            if (!File.Exists(_selectedPath + "\\" + _name + ".sqlite"))
            {
                SQLiteConnection.CreateFile(_selectedPath + "\\" + _name + ".sqlite");
                Database = new ConnectSQLite(_selectedPath + "\\" + _name + ".sqlite");
                Database.OuvrirBDD();
                ...
                SetCurrent(_selectedPath, _name);
                XML_AddToHistory(_selectedPath, _name);
        ...
        private void SetCurrent(string _path, string _name)
        {
            ...
            if (!File.Exists(_path + "\\" + _name + ".sqlite"))
                Database = null;
            else
            {
                Database = new ConnectSQLite(_path + "\\" + _name + ".sqlite");
                Database.OuvrirBDD();
                GetAllBooks();
            }
            if (FormBase != null)
                FormBase.LBL_NomBib.Text = _name;
        }
        ...
        private void OpenFolderBrowser(object sender, EventArgs e)
        {
            ...
            {
                Dialog.LBL_CurrentPath.Text = "Emplacement Actuel : " + folderDlg.FileName;
                Dialog.TXT_Path.Text = folderDlg.FileName;
            }
        }
